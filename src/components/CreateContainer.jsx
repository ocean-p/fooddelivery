...
import {
  MdFastfood, MdCloudUpload, MdDelete, MdFoodBank,
  MdAttachMoney
} from 'react-icons/md';
...
const CreateContainer = () => {
  ...
  const saveDetails = () => {
    ...
    try {
      if (!title || !calories || !imageAsset || !price || !category) {
        ...
        setIsLoading(false);
        setFields(true);
        setMsg('Saving item successfully 🤩.');
        setAlertStatus('success');
        setTimeout(() => {
          setFields(false);
        }, 4000);
  ...
  return (
    <div className='w-full min-h-screen flex items-center justify-center'>
      <div className='w-[90%] md:w-[65%] border border-gray-300 rounded-lg
      ...
      >
        ...
        <div className='group flex justify-center items-center flex-col border-2
          border-dotted border-gray-300 w-full h-225 md:h-420 cursor-pointer
          rounded-lg'
        >
          {isLoading ? <Loader /> : <>
            {!imageAsset ? (<>
        ...
        <div className='flex items-center w-full'>
          <button
            ...
            className='ml-0 md:ml-auto w-full md:w-auto border-none outline-none
              bg-emerald-500 px-12 py-2 rounded-lg text-lg text-white
              font-semibold'
            onClick={saveDetails}
          >
            Save
